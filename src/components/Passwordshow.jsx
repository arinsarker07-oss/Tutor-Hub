...
export default function PasswordShow() {
  ...
  return (
    <div className="relative flex items-center group">
      <FaLock className="absolute left-4 text-[#237888] dark:text-teal-400 text-sm" />
      <input 
        type={showPassword ? "text" : "password"} 
        name="password"
        required
        pattern="^(?=.*[a-z])(?=.*[A-Z]).{6,}$"
        title="Must contain at least 6 characters, including one uppercase and one lowercase letter."
        placeholder="••••••••"
